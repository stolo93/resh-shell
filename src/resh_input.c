...
int parseArgs(char * line, args_t * args)
{
    ...
    args->args_count = arg_count;

    args->args = malloc(sizeof(char *) * (args->args_count + 1)); //+1 to end the array with NULL
    ...
    int stored_args = 0;
    do {
        args->args[stored_args++] = tmp; 


    } while ((tmp = strtok(NULL, " ")));
    
    args->args[stored_args] = NULL;
